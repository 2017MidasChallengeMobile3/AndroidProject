...
public class DonationDBHelper extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        ...
        final String SQL_CREATE_COMPLETE_TABLE = "CREATE TABLE DONATION (" +
    ...
    public void insert(int userCode, int vsCode) {
        ...
        // DB에 입력한 값으로 행 추가
        db.execSQL("INSERT INTO DONATION(user_code, vs_code) " +
    ...
    public void delete(int completeCode) {
        SQLiteDatabase db = getWritableDatabase();
        // 입력한 항목과 일치하는 행 삭제
        db.execSQL("DELETE FROM DONATION WHERE complete_code = " + completeCode + ";");
        db.close();
    }

    //select = 사용자가 봉사활동 완료한 리스트 반환
    public ArrayList<Donation> selectComplete(int userCode) {
        // 읽기가 가능하게 DB 열기
